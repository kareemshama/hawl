...
import { clamp01, round2 } from "./util.js";
...
export function valueAsset(asset: Asset, ctx: Ctx): Trace {
  ...
  const finish = (input: number, zakatable: number, rules: string[], extra: Partial<Trace> = {}): Trace => ({
    ...base,
    input: round2(input),
    zakatable: round2(zakatable * share),
  ...
  switch (asset.kind) {
    ...
    case "metal": {
      const pricePerGram = asset.metal === "gold" ? ctx.prices.goldPerGram : ctx.prices.silverPerGram;
      ...
      return finish(value, value, rules, {
        note: `${asset.grams} g at ${Math.round(clamp01(asset.purity) * 1000) / 10} percent purity x ${pricePerGram} ${ctx.prices.currency}/g`,
      });
    }
    ...
    case "retirement": {
      ...
      switch (ctx.settings.retirementMethod) {
        case "net-accessible": {
          const penalty = clamp01(asset.penaltyRate ?? 0);
          const tax = clamp01(asset.taxRate ?? 0);
          const net = Math.max(0, vested * (1 - penalty - tax) - (asset.fees ?? 0));
          return finish(vested, net, ["R8.1", "R8.2", "R8.3"], {
            note: `Vested balance minus ${Math.round(penalty * 100)} percent penalty, ${Math.round(tax * 100)} percent tax, and ${round2(asset.fees ?? 0)} fees.${unvestedNote}`,
          });
        }
        case "long-term-proportional": {
          const ratio = asset.underlyingZakatableRatio ?? ctx.settings.stockProxyPercent / 100;
          const usedProxy = asset.underlyingZakatableRatio === undefined;
          return finish(vested, vested * clamp01(ratio), ["R8.2", "R8.3", "R9.2"], {
            note: `Treated as a long-term investment at ${Math.round(clamp01(ratio) * 1000) / 10} percent zakatable.${unvestedNote}`,
            estimated: usedProxy,
          });
        }
        case "strict-full":
    ...
    case "stock": {
      ...
      const ratio = asset.zakatableAssetRatio ?? ctx.settings.stockProxyPercent / 100;
      const usedProxy = asset.zakatableAssetRatio === undefined;
      return finish(asset.marketValue, asset.marketValue * clamp01(ratio), ["R9.2"], {
        note: usedProxy
          ? `Long-term holding at the ${ctx.settings.stockProxyPercent} percent proxy.`
          : `Long-term holding at the exact zakatable-asset ratio of ${Math.round(clamp01(ratio) * 1000) / 10} percent.`,
        estimated: usedProxy,
      });
    }
    ...
    case "business": {
      ...
      return finish(gross, Math.max(0, net), ["R12.1", "R12.2", "R12.3"], { note: parts.join(", ") });
    }
    ...
    case "other-account":
      switch (asset.subtype) {
        ...
        case "education-529": {
          const penalty = clamp01(asset.penaltyRate ?? 0);
          const tax = clamp01(asset.taxRate ?? 0);
          return finish(asset.balance, asset.balance * (1 - penalty - tax), ["R13.4", "R8.2"], {
            note: "Parent-controlled education account, net of penalty and tax.",
          });
        }
        case "junior-isa":
  ...
  // Exhaustiveness guard. TypeScript narrows `asset` to never above when every case returns.
  const unreachable: never = asset as never;
  throw new Error(`Unhandled asset: ${JSON.stringify(unreachable)}`);
}
